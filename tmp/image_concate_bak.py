...
def distance(bbx1, bbx2):
    aa = bbx1 - bbx2
    # print("distance is ", aa)
    bb = np.sqrt(np.sum(np.square(bbx1 - bbx2)))
    # print(" bb is ", bb)
    return bb


def concate_image(img1, img2, from_y, to_y, overwrite=False):
    """
    将image进行拼接
    ...
    """
    img = []
    if from_y == 0 and overwrite == True:
        print("overwrite ...")

        img1[from_y:to_y, :, :] = img2[0:to_y, :, :]
        img = img1
    elif from_y == 0:
        print("concate from zero")
        # img1[from_y:to_y, :] = img2
        img = np.vstack((img1, img2))
        print("merge from {} to {}".format(0, to_y))
    elif from_y < to_y:
        img = np.vstack((img1, img2[from_y:to_y, :]))
...
def concate_or_replace_image(image1, image2):
    ...
    if min_dis == 0:
        ...
        if min_index < y2:
            print("concate image from {} to {}", min_index, image2H )
            image = concate_image(image1, image2, min_index, image2H )
            # cv2.imshow('image', image)
            cv2.imwrite(result_images.format(time.time()), image)
        else:
            print(" overwrite image from {} to {}".format(0, image2H ))
        ...
        image = concate_image(image1, image2, min_index, image2H , overwrite=False)
        cv2.imwrite(result_images .format(time.time()), image)
        print("iterate to the end y:", y2, " and find min distance ")
